...
//}
void forceCallback(const geometry_msgs::Vector3ConstPtr& msg)
{	
    boost::array<double,3> forces;
    forces[0] = msg->x;
    forces[1] = msg->y;
    forces[2] = msg->z;
            std::cout<< "Fx is "<<forces[0]<< std::endl;
	m_falconDevice.setForce(forces);
}
int main(int argc, char* argv[])
{
    ...
    { 
       ...
       m_falconDevice.setFalconKinematic<libnifalcon::FalconKinematicStamper>();
        ...
        ros::Publisher pub = node.advertise<rosfalcon::falconPos>("/falconPos",100);
	ros::Rate loop_rate(1000); 
        ...
        //ros::Subscriber sub = node.subscribe("/falconForces", 1, &forceCallback); 
	    ...
	    a=Pos[2];
            b=Pos[0];
	    c=Pos[1];
	    a=floor(a*1000.000f)/1000.000f;
	    b=floor(b*1000.000f)/1000.000f;
	    c=floor(c*1000.000f)/1000.000f;
            //position.X = -0.1*a+0.12-0.215;
            //position.Y = -0.1*b+0.2;
            //position.Z = 0.1*c+0.2;
            position.X = -0.5*a+0.062;
            position.Y = -0.5*b;
            position.Z =  0.5*c;

            pub.publish(position);

            //std::cout << "Vx="<< position.X<< "  Vy="<< position.Y<< "  Vz="<< position.Z << std::endl;          
	    }
	    loop_rate.sleep();
	    }
