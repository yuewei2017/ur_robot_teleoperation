...
public:
	RosWrapper(std::string host, int reverse_port) :
		...
		if (robot_.start()) {
			...
			managerThread = std::thread(&RosWrapper::manager, this);
		}
	}
...
private:
	void manager(void)
	{
		while(ros::ok())
		{
			if(!(robot_.rt_interface_->connected_) || !(robot_.sec_interface_->connected_)) 
			{
				robot_.halt();
				ROS_INFO("robot lost.");
	...
	void trajThread(std::vector<double> timestamps,
		...
		robot_.doTraj(trac_state, timestamps, positions, velocities);
		print_info("running");
		if (has_goal_) {
	...
	void publishRTMsg() {
		...
		while (ros::ok()) {
			...
			char mStr[10];
			sprintf(mStr, "%d", trac_state);
			joint_msg.header.frame_id = mStr;
	...
	void publishMbMsg() {
		...
		while (ros::ok()) {
			//if(!(robot_.rt_interface_->connected_)) {ros::shutdown();return;}
			//if(!(robot_.sec_interface_->connected_)) {ros::shutdown();return;}
