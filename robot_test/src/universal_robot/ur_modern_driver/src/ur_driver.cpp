...
std::vector<double> UrDriver::interp_cubic(double t, double T, double T0,
		std::vector<double> p0_pos, std::vector<double> p1_pos,
		std::vector<double> p0_vel, std::vector<double> p1_vel) {
	/*Returns positions of the joints at time 't' */
	std::vector<double> positions;
	for (unsigned int i = 0; i < p0_pos.size(); i++) {
		double a = p0_pos[i];
		double b = p1_pos[i] - p0_pos[i];
		double c = T - T0;
		double d = t - T0;
		positions.push_back(a + b * d / c);
	}
	return positions;
}

bool UrDriver::doTraj(char &Traj_state, std::vector<double> inp_timestamps,
	...
	t = t0;
	j = 0;
	ROS_INFO("position num:%d.", (int)inp_timestamps.size());
	while ((inp_timestamps[inp_timestamps.size() - 1]
		...
		if(inp_timestamps.size() <= 200)
			positions = UrDriver::interp_cubic(
					...
					inp_velocities[j - 1], 
					inp_velocities[j]);
		else if(inp_timestamps.size() > 200)
			positions = UrDriver::interp_cubic(
					std::chrono::duration_cast<std::chrono::duration<double>>(t - t0).count() - inp_timestamps[j - 1],
					inp_timestamps[j],
					inp_timestamps[j - 1], 
