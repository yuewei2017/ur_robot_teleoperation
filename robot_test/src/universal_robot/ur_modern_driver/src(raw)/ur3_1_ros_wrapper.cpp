...
public:
  RosWrapper(std::string host, int reverse_port) :
      as_(nh_, "follow_joint_trajectory_ur3_1",
...
private:
  ...
  void publishRTMsg() {
    ros::Publisher joint_pub = nh_.advertise<sensor_msgs::JointState>(
        "ur3_1/joint_states", 1);
    ros::Publisher wrench_pub = nh_.advertise<geometry_msgs::WrenchStamped>(
        "ur3_1/wrench", 1);
        ros::Publisher tool_vel_pub = nh_.advertise<geometry_msgs::TwistStamped>("ur3_1/tool_velocity", 1);
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "ur3_1_driver");
  ros::NodeHandle nh;
